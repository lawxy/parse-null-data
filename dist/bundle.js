...
function ProxyFun(val) {
  // 代理函数初始值， 可以替换
  var returnVal = val; // 执行最终函数时，判断是否强替换， 否则判断使用初始值或者默认值

  var fun = function fun(defaultVal, replace) {
    var result;
    if (replace === true) result = defaultVal;else result = returnVal ? returnVal : defaultVal;
    return result;
...
function proxyObj(obj, defaultVal) {
  var fun = new ProxyFun(defaultVal);
  return new Proxy(fun, {
    get: function get(target, key) {
      // fun() = {b:123}
      if (_typeof(obj[key]) === 'object' && obj[key] !== null || typeof obj[key] === 'funtion') {
        for (var k in obj[key]) {
          fun[k] = obj[key][k];
        }
      }

      var p = proxyObj(fun, obj[key]);
